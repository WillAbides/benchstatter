...
func Test_refRunner_run(t *testing.T) {
	...
	err := ioutil.WriteFile(fooPath, []byte("OG content"), 0600)
	require.NoError(t, err)
	mustGit(t, dir, "init")
	mustGit(t, dir, "add", "foo")
	mustGit(t, dir, "commit", "-m", "ignore me")
	untrackedPath := filepath.Join(dir, "untracked")
	err = ioutil.WriteFile(untrackedPath, []byte("untracked"), 0600)
	require.NoError(t, err)
	err = ioutil.WriteFile(fooPath, []byte("new content"), 0600)
	require.NoError(t, err)
	fn := func() {
